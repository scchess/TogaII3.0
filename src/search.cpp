		   ...
		   for (SearchCurrent[ThreadId]->multipv = 0; SearchCurrent[ThreadId]->multipv <= SearchInput->multipv; SearchCurrent[ThreadId]->multipv++){
              ...
              while (1){
    		    ...
    		    if (UseShortSearch && depth <= ShortSearchDepth) {
    		  	   search_full_root(SearchRoot[ThreadId]->list,SearchCurrent[ThreadId]->board,a,b,depth,SearchShort,ThreadId);
    		    } else {
    			   search_full_root(SearchRoot[ThreadId]->list,SearchCurrent[ThreadId]->board,a,b,depth,SearchNormal,ThreadId);
    		    }
    		    ...
    		    if (SearchBest[ThreadId]->value <= a){
                    a -= delta;
                    delta += delta;
                } else if (SearchBest[ThreadId]->value >= b){
                    b += delta;
                    delta += delta;
                } else {
                    a = SearchBest[ThreadId]->value-delta;
                    b = SearchBest[ThreadId]->value+delta; 
                    break;
                }    
              }
