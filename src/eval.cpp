...
static const int bishop_mob_opening[8 + 1] = {-20, -15, -10,  -5,   0,   5,   10,  15,  20};                         // WHM; slope == BishopMobOpening == 5
static const int bishop_mob_endgame[8 + 1] = {-20, -15, -10,  -5,   0,   5,   10,  15,  20};                         // WHM; slope == BishopMobEndgame == 5
   ...
   for (colour = 0; colour < ColourNb; colour++) {
      ...
      for (ptr = &board->piece[me][1]; (from=*ptr) != SquareNone; ptr++) { // HACK: no king
         ...
         case Bishop64:
            ...
            // bishop mobility only considers forward mobility + 1 square for retreat
            // probably a very small elo gain
            ...
            if (colour == White){
                ...
                if (board->square[from-15] != WP ||  board->square[from-17] != WP) mob++;
                ...
                if (board->square[from+15] != BP ||  board->square[from+17] != BP) mob++;
         ...
         case Queen64:
            ...
            // mobility
            // we see how close we are to the opponent's king
